...
namespace XFS4IoT::CashManagement
{
    ...
    private:
        // unrecognized_ - это количество нераспознанных предметов, обработанных денежным интерфейсом. Это может быть null в данных команды и событиях, если не изменилось или не должно измениться. Минимальное значение: 0. Это позволяет отслеживать к��личество предметов, которые были обработаны, но не распознаны устройством, что может быть полезно для диагностики и отчетности.
        std::optional<int> unrecognized_;
        // это позволяет хранить дополнительные свойства с произвольными ключами, например, 
        // для хранения количества по каждой валюте/номиналу, если это поддерживается устройством. 
        // Ключ - это строка, которая может быть, например, "typeEUR5" для 5 евро или "typeUSD20" для 20 долларов.
        // Значение - это объект StorageCashCountClass, который содержит количество fit/unfit/suspect/counterfeit/inked 
        // для этого конкретного типа банкноты. Это позволяет расширять модель данных без изменения структуры класса и 
        // поддерживать произвольные дополнительные данные от устройств. 
