...
namespace XFS4IoTServer
{
    ...
    void ServicePublisher::Initialize()
    {
        ...
        uint16_t specificPort = 0;
        specificPort = SettingModule::GetInstance()->getWSPort();
		if (specificPort) {
			try {
				auto it = std::ranges::find(portRanges, specificPort);
				if (it != portRanges.end()) {
					portRanges.erase(it);
					portRanges.insert(portRanges.begin(), specificPort);
				}
				else {
					logger_->warn(
        ...
        for (int port : portRanges) {
            try {
                ...
                rawDecoder->RegisterMessageType<XFS4IoT::ServicePublisher::Commands::GetServicesCommand>(
                    XFS4IoT::ServicePublisher::Commands::GetServicesCommand::CommandName,
                    {XFS4IoT::ServicePublisher::Commands::GetServicesCommand::Version/*"1.5"*/, "2.0", "3.5"});
